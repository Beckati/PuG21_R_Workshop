...
```{r}
hotdog_men <- hotdog_2019[hotdog_genders == "male"]
hotdog_men
hotdog_women <- hotdog_2018[hotdog_genders == "female"]
hotdog_women
```
...
female_results
max_woman <- female_names[female_results == max(female_results)]
max_woman
```
...
```{r}
min(c(1,2,3,NA,5))
min(c(1,2,3,NA,5), na.rm = TRUE)

max(c(1,2,3,NA,5))
max(c(1,2,3,NA,5), na.rm = TRUE)
```


5.3) `summary()`
```{r}
summary(c(1,2,3,NA,5))
summary(c(1,2,3,NA,5), na.rm = TRUE)
```
...
```{r}
head(c(1,2,3,NA,5))
head(c(1,2,3,NA,5), na.rm = TRUE)

tail(c(1,2,3,NA,5))
tail(c(1,2,3,NA,5), na.rm = TRUE)
```
...
```{r}
length(c(1,2,3,NA,5))
length(c(1,2,3,NA,5), na.rm = TRUE)
```
