...
nerd_red <-
  nerd_red %>%
  mutate(age_character = as.integer(age), .before = gender)
```
...
```{r}
nerd_red %>%
  mutate(gender_correct = (gender +1), .before = gender)
nerd_red %>%
  mutate(gender = (gender +1), .before = married)
```
